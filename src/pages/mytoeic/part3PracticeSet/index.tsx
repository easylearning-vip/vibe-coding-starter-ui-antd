...
} from '@/services/part3-practice-set/api';
...
type Part3PracticeSet = Part3PracticeSetAPI.Part3PracticeSet;

const Part3PracticeSetManagement: React.FC = () => {
  const [practiceSets, setPracticeSets] = useState<Part3PracticeSet[]>([]);
  ...
  const handleDelete = async (practiceSet: Part3PracticeSet) => {
    if (!practiceSet.id) {
  ...
  // 查看练习集详情
  const handleViewDetails = async (practiceSet: Part3PracticeSet) => {
            ...
            <div style={{ maxHeight: 400, overflow: 'auto' }}>
              {questions.map((q, index) => {
                ...
                switch (q.question_index) {
                  case 1:
                    questionText = safeRender(q.conversation?.question1) || '';
                    break;
                  case 2:
                    questionText = safeRender(q.conversation?.question2) || '';
                    break;
                  case 3:
                    questionText = safeRender(q.conversation?.question3) || '';
                    break;
                  default:
                    questionText = '';
                }
                ...
                const answerOption = q.conversation?.answer_options?.find(
  ...
  // 开始练习
  const handleStartPractice = (practiceSet: Part3PracticeSet) => {
    if (!practiceSet.id) {
  ...
  return (
    <PageContainer
      title="Part 3 Practice Sets"
      content="Manage your personal TOEIC Part 3 practice sets"
    >
      <Card>
        {/* 搜索表单 */}
        <Form
        ...
        <List
          ...
          renderItem={(item: Part3PracticeSet) => {
      ...
      <AddToPracticeSetModal
        ...
        toeicPart="part3"
        title="Create Part 3 Practice Set"
      />
...
export default Part3PracticeSetManagement;
