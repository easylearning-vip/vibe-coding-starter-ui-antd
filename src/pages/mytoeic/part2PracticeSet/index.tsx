...
} from '@/services/part2-practice-set/api';
...
type Part2PracticeSet = Part2PracticeSetAPI.Part2PracticeSet;

const Part2PracticeSetManagement: React.FC = () => {
  const [practiceSets, setPracticeSets] = useState<Part2PracticeSet[]>([]);
  ...
  const handleDelete = async (practiceSet: Part2PracticeSet) => {
    if (!practiceSet.id) {
  ...
  // 查看练习集详情
  const handleViewDetails = async (practiceSet: Part2PracticeSet) => {
            ...
            <div style={{ maxHeight: 400, overflow: 'auto' }}>
              {questions.map((q, index) => (
                <div key={q.item_id} style={{ marginBottom: 16, padding: 12, border: '1px solid #f0f0f0', borderRadius: 4 }}>
                  <p><strong>Q{index + 1}:</strong> {safeRender(q.question?.question_text)}</p>
                  <p>A: {safeRender(q.question?.option_a)}</p>
                  <p>B: {safeRender(q.question?.option_b)}</p>
                  <p>C: {safeRender(q.question?.option_c)}</p>
                  <p><strong>Answer:</strong> {safeRender(q.question?.correct_answer)}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  // 开始练习
  const handleStartPractice = (practiceSet: Part2PracticeSet) => {
    if (!practiceSet.id) {
  ...
  return (
    <PageContainer
      title="Part 2 Practice Sets"
      content="Manage your personal TOEIC Part 2 practice sets"
    >
      <Card>
        {/* 搜索表单 */}
        <Form
      ...
      <AddToPracticeSetModal
        ...
        toeicPart="part2"
        title="Create Part 2 Practice Set"
      />
...
export default Part2PracticeSetManagement;
