...
import { useArticleStatus } from '@/hooks/useDict';
...
const ArticleManagement: React.FC = () => {
  ...
  // 使用数据字典Hook
  const _articleStatus = useArticleStatus();
  ...
  const fetchArticles = async (params?: {
    ...
    try {
      ...
      const response = await getAdminArticleList(queryParams);
  ...
  const fetchUsers = async () => {
    try {
      const response = await getUserList({ page: 1, page_size: 100 });
  ...
  return (
    <PageContainer
      title={intl.formatMessage({ id: 'pages.admin.articles.title' })}
      content={intl.formatMessage({ id: 'pages.admin.articles.description' })}
    >
      <Card>
        {/* 搜索表单 */}
        <Form
        ...
        >
          <Row gutter={[16, 16]} style={{ width: '100%' }}>
            ...
            <Col span={4}>
              <Form.Item
                name="author_id"
                label={intl.formatMessage({ id: 'pages.common.author' })}
              >
                <Select
                  placeholder={intl.formatMessage({
                    id: 'pages.admin.articles.search.author.placeholder',
                  })}
                  allowClear
                  showSearch
